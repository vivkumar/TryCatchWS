...
public class UTS {
    ...
    public void runIteration() {
        final Node rootNode = Config.rootNode;
        final Queue<Node> workingCopy = new LinkedList<Node>();
        final AtomicBoolean eurekaObject = new AtomicBoolean();
        workingCopy.offer(rootNode);
        finish{
            findElement(workingCopy, eurekaObject);
        }
        if (!eurekaObject.get()) {
    ...
    private void findElement(final Object loopQueue, final AtomicBoolean eurekaObject) {
        final Queue<Node> workingCopy = (Queue<Node>) loopQueue;
        while (!workingCopy.isEmpty()) {
            final Node loopNode = workingCopy.poll();
            if (Config.foundElement(loopNode)) {
                eurekaObject.set(true);
                return;
            }
            addChildren(loopNode, workingCopy);
            processPendingNodes(workingCopy, eurekaObject);
        }
    }

    private void processPendingNodes_wrapper(Object[] queues, final AtomicBoolean eurekaObject) {
        async {
          for (int i=0; i<queues.length; i++) { 
            findElement(queues[i], eurekaObject);
          }
        }
    }

    private void processPendingNodes(final Queue<Node> workingCopy, final AtomicBoolean eurekaObject) {
        if (!workingCopy.isEmpty()) {
            final Queue<Node>[] queues = split(workingCopy);
            processPendingNodes_wrapper(queues, eurekaObject);
        }
    }

    private void addChildren(final Node treeNode, final Queue<Node> workingCopy) {
        final int childCount = treeNode.numChildren();
        for (int i = 0; i < childCount; i++) {
            final Node childNode = treeNode.child(i);
            workingCopy.offer(childNode);
        }
    }
   ...
   /**
    ...
    protected static <T> Queue<T>[] split(final Queue<T> list) {
        ...
        final Queue<T>[] retValue = new LinkedList[]{new LinkedList<Node>(), new LinkedList<Node>()};
        while (!list.isEmpty()) {
...
class Config {
    ...
    protected static int nodeGranularity = 1000000;
    protected static int checkGranularity = 10000;
    protected static int maxHeight = 10;
    protected static double itemToFindLoc = 0.50;
    ...
    protected static void parseArgs(final String[] args) {
        ...
        while (i < args.length) {
            ...
            if(loopOptionKey.equals("-ng")) {
            	i += 1;
            	nodeGranularity = Integer.parseInt(args[i]);
            }
            if(loopOptionKey.equals("-cg")) {
            	i += 1;
            	checkGranularity = Integer.parseInt(args[i]);
            }
    ...
    protected static void printArgs() {
        ...
        System.out.println("Node Granularity "+ nodeGranularity);
        System.out.println("Check Granularity "+ checkGranularity);
