...
public class Sequential implements Config.Benchmark {
  public static void main(final String[] args) {
    ...
    Sequential benchmark = new Sequential(numCities, goal);
  ...
  public Sequential(int nc, int goal) {
    numCities = nc;
  ...
  private void speculativeSearch(final int size, byte[] nextCities, final boolean[] blocked, final int level, final int lowerBound) {
    ...
    for (byte city = 2; city <= numCities; city++) {
      if (!Config.present(city, nextCities)) {
        ...
        speculativeSearch(newSize, newTour, newBlocked, newLevel, newLowerBound);
      }
    }
  }
}
