...
public class Config {
  ...
  // ShortlongPath parameters and defaults
  protected static int treeType = GEO;            // ShortlongPath Type: Default = GEO
  ...
  //protected static int goalRate = 20000;
  //protected static int SHORTEST_PATH_GOAL = 2463;
  //protected static int LONGEST_PATH_GOAL = 8005;
  protected static int goalRate = 25000;
  protected static int SHORTEST_PATH_GOAL = 1058;
  protected static int LONGEST_PATH_GOAL = 7563;

  protected static Set<SPNode> goalSPNodes = new HashSet<SPNode>();
  ...
  protected static void setup(String _type, int _goalRate) {
    type = _type;
    goalRate = _goalRate;
  ...
  private static void initialize(final String type) {
    ...
    final int numSPNodes = countSPNodes(rootSPNode);
    System.out.println("Total nodes = "+numSPNodes);
    AtomicInteger nodecount = new AtomicInteger(1);
    addGoalSPNodes(rootSPNode, nodecount);
    System.out.println("Total nodes = "+numSPNodes+" Goal SPNodes= "+goalSPNodes.size()+" Goal Rate ="+goalRate);
  }
  ...
  private static void addGoalSPNodes(final SPNode node, final AtomicInteger nodecount) {
    final int count = nodecount.incrementAndGet();
    if(count%goalRate ==0) {
      goalSPNodes.add(node);
      return; // try some other path
    }
    final int numChildren = node.nChildren;
    for (int n = 0; n < numChildren; n++) {
      final SPNode childSPNode = node.child(n);
      addGoalSPNodes(childSPNode, nodecount);
    }
  }
  ...
  static class SPNode {
    ...
    SPNode(final Sha1Generator state, final int height) {
      this.state = state;
    ...
    private void initializeChildren() {
      final MyRandom random = new MyRandom(height * nChildren);
      for (int c = 0; c < nChildren; c++) {
        edgeWeights[c] = (random.nextInt()) % 1023;
