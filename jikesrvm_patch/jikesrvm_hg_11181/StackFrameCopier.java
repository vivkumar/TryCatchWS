...
public final class StackFrameCopier {
	...
	protected void copyStack(Address fp, Address ip, RVMThread scanThread, RVMThread toThread, 
		...
		Memory.alignedWordCopy(prevFp.plus(newStackOffset).plus(SizeConstants.BYTES_IN_ADDRESS), prevFp.plus(SizeConstants.BYTES_IN_ADDRESS), fp.diff(prevFp).toInt() - SizeConstants.BYTES_IN_ADDRESS);
		...
		while(true) {
			...
			if (nextFp.EQ(ArchitectureSpecific.StackframeLayoutConstants.STACKFRAME_SENTINEL_FP) || count == 0) {
				...
				prevFp.plus(newStackOffset).store(Address.zero(), Offset.fromIntSignExtend(ArchitectureSpecific.StackframeLayoutConstants.STACKFRAME_RETURN_ADDRESS_OFFSET.toInt()));
				fp.plus(newStackOffset).store(ArchitectureSpecific.StackframeLayoutConstants.INVISIBLE_METHOD_ID, Offset.fromIntSignExtend(ArchitectureSpecific.StackframeLayoutConstants.STACKFRAME_METHOD_ID_OFFSET.toInt()));
				break;
			}

			// Copy the rest of this frame
			Memory.alignedWordCopy(fp.plus(newStackOffset).plus(SizeConstants.BYTES_IN_ADDRESS), fp.plus(SizeConstants.BYTES_IN_ADDRESS), nextFp.diff(prevFp).toInt() - SizeConstants.BYTES_IN_ADDRESS);
